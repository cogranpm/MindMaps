...
import { FETCH_ID_LOADMINDMAPS, FETCH_ID_SYNC } from "~src/shared/constants";
...
export function SyncDialog(props: SyncDialogProps) {
  ...
  async function handleSubmit() {
    try {
      ...
      await replicate(getPreference(PREFERENCE_COUCHURL), {
        auth: { username: userName, password: password },
      });

      const items = await loadAll();
      dispatch({ type: ActionType.InitList, payload: { items: items } });
    } catch (err) {
      let errorMsg = "";
      if (err instanceof SyncError) {
        //const errorInfo = err.errorInfo.info.map((info) => info.type);
        //errorMsg = errorMsg + errorInfo.join(": ");
      ...
      setIsError(true);
      logSystemError(err, "Error Syncing");
    } finally {
      setComplete(true);
      setInProgress(false);
    }
...
*/
  ...
  return (
    <Modal size="xl" show={true}>
      ...
      <Modal.Footer>
        <button title="Close" onClick={() => props.setShow(false)}>
          Close
        </button>
        <button title="Sync" onClick={handleSubmit} disabled={inProgress}>
          Begin Sync
        </button>
      </Modal.Footer>
    </Modal>
