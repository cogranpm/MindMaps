...
//import { get, load, remove } from "../models/mindMaps/repository";
...
export function List() {
  ...
  const handleSelection = async (item: MindMapViewModel) => {
    ...
    /*
    try {
      const entity = await load(item._id);
      dispatch({ type: ActionType.OpenTab, payload: { entity: entity } });
    } catch (e) {
      logSystemError(e, "Error loading mind map");
    }
    */
  };
  ...
  const removeMindMap = async (item: MindMapViewModel) => {
    ...
    try {
      //remove(item._id);
      //dispatch({ type: ActionType.Remove, payload: { item: item } });
    ...
    <Table striped bordered hover size="sm">
      ...
      <thead>
        <tr>
          <th>
            <ButtonGroup size="sm">
              <Button variant="outline-info" onClick={create}>
                Create
              </Button>
            </ButtonGroup>
          </th>
